...
class TemporalGrid:
    ...
    def stable_components(
        ...
        stable: Set[Cell] = {
            cell for cell, count in self._cell_frames.items() if count >= threshold
        }
        ...
        while stable:
            ...
            while stack:
                ...
                for dx in range(-connection_radius_cells, connection_radius_cells + 1):
                    for dy in range(-connection_radius_cells, connection_radius_cells + 1):
        ...
        for component in components:
            ...
            peak = max(self._cell_frames[cell] / self.frame_count for cell in component)
            result.append(
                StableComponent(
                    x_m=float(median(xs)),
                    y_m=float(median(ys)),
...
def estimate_arrangement_axis(
    ...
    covariance_xx = sum((item.x_m - mean_x) ** 2 for item in components)
    covariance_yy = sum((item.y_m - mean_y) ** 2 for item in components)
    covariance_xy = sum((item.x_m - mean_x) * (item.y_m - mean_y) for item in components)
...
def compensate_target_centers(
    ...
    if not components:
        return []
    if len(along_offsets_m) != len(components) or len(normal_offsets_m) != len(components):
    ...
    corrected = []
    for component, along, normal in zip(components, along_offsets_m, normal_offsets_m, strict=True):
        corrected.append(
            StableComponent(
                x_m=(component.x_m + float(along) * axis_x + float(normal) * normal_x),
                y_m=(component.y_m + float(along) * axis_y + float(normal) * normal_y),
...
def compensate_polar_coordinates(
    ...
    for component in components:
        ...
        corrected_range_m = range_m + sum(
            ...
            for coefficient, feature in zip(range_coefficients_m, features, strict=True)
        )
        corrected_angle_rad = angle_rad + sum(
            float(coefficient) * feature
            for coefficient, feature in zip(angle_coefficients_rad, features, strict=True)
        )
        corrected.append(
            StableComponent(
...
def select_target_components(
    ...
    candidates = [
        component for component in components if component.max_span_m <= maximum_component_span_m
    ]
    ...
    for subset in combinations(candidates, expected_count):
        ...
        if axis_name == "auto":
            ...
            normal_x, normal_y = -axis_y, axis_x
            along = sorted(item.x_m * axis_x + item.y_m * axis_y for item in subset)
            across = [item.x_m * normal_x + item.y_m * normal_y for item in subset]
        elif axis_name == "x":
        ...
        across_residual = sum(abs(value - across_center) for value in across) / expected_count
        ...
        spacing_penalty = 0.0
        if expected_count >= 3:
            spacings = [along[index] - along[index - 1] for index in range(1, len(along))]
            ...
            if (
                ...
                and maximum_deviation_ratio > maximum_spacing_deviation_ratio
            ):
                continue
            spacing_penalty = sum(abs(value - mean_spacing) for value in spacings) / len(spacings)
...
def split_targets_have_plausible_parent(
    ...
    for parent in standard_components:
        ...
        if all(
            abs(target.x_m - parent.x_m) <= half_x and abs(target.y_m - parent.y_m) <= half_y
    ...
    for target in split_targets:
        ...
        for index, parent in enumerate(standard_components):
            ...
            half_x = 0.5 * parent.span_x_m + tolerance_m
            half_y = 0.5 * parent.span_y_m + tolerance_m
            if abs(target.x_m - parent.x_m) <= half_x and abs(target.y_m - parent.y_m) <= half_y:
                matching.append(
                    (
                        math.hypot(
...
def arrangement_alignment_error(
    ...
    axis_x, axis_y = estimate_arrangement_axis(components)
    normal_x, normal_y = -axis_y, axis_x
    across = [item.x_m * normal_x + item.y_m * normal_y for item in components]
...
def arrangement_spacing_metrics(
    ...
    axis_x, axis_y = estimate_arrangement_axis(components)
    along = sorted(item.x_m * axis_x + item.y_m * axis_y for item in components)
    spacings = [along[index] - along[index - 1] for index in range(1, len(along))]
    mean_spacing = sum(spacings) / len(spacings)
    if mean_spacing <= 0.0:
        return 0.0, math.inf
    maximum_deviation_ratio = max(abs(value - mean_spacing) / mean_spacing for value in spacings)
