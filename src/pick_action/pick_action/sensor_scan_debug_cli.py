...
class SensorScanDebugCli(Node):
    ...
    def wait_for_tool_service(self) -> bool:
        return self._tool_client.wait_for_service(timeout_sec=self.args.service_wait_timeout_s)
    ...
    def call_tool(self, action: str, tool_args: list[float], timeout_s: float) -> bool:
        ...
        req.args = tool_args[:4] + [0.0] * max(0, 4 - len(tool_args))
        print(
            "发送同步工具命令: action=%s args=%s timeout=%.3fs"
            % (
                req.action,
                req.args,
                timeout_s,
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sensor-topic", default=_param(params, "sensor_topic", "/sensor_distances")
    )
    parser.add_argument(
        "--tool-service", default=_param(params, "tool_service", "/ares_tool_node/tool_action")
    )
    ...
    parser.add_argument(
        "--scan-sensor-index", type=int, default=int(_param(params, "scan_sensor_index", 1))
    )
    parser.add_argument(
        "--scan-sensor-max-age-s",
        type=float,
        default=float(_param(params, "scan_sensor_max_age_s", 0.5)),
    )
    parser.add_argument(
        "--scan-enable-jump-trigger",
        action=argparse.BooleanOptionalAction,
        default=bool(_param(params, "scan_enable_jump_trigger", False)),
    )
    parser.add_argument(
        "--scan-jump-threshold-mm",
        type=float,
        default=float(_param(params, "scan_jump_threshold_mm", 300.0)),
    )
    parser.add_argument(
        "--scan-present-threshold-mm",
        type=float,
        default=float(_param(params, "scan_present_threshold_mm", 150.0)),
    )
    parser.add_argument(
        "--scan-present-duration-s",
        type=float,
        default=float(_param(params, "scan_present_duration_s", 0.1)),
    )
    parser.add_argument(
        "--debug-pre-scan-position-m",
        type=float,
        default=float(_param(params, "debug_pre_scan_position_m", 0.45)),
    )
    parser.add_argument(
        "--debug-scan-target-position-m",
        type=float,
        default=float(_param(params, "debug_scan_target_position_m", 0.1)),
    )
    parser.add_argument(
        "--debug-pre-scan-timeout-s",
        type=float,
        default=float(_param(params, "prepare_timeout_ms", 20000)) / 1000.0,
    )
    parser.add_argument(
        "--scan-prepare-length-m",
        type=float,
        default=float(_param(params, "scan_prepare_length_m", 0.05)),
    )
    parser.add_argument(
        "--scan-home-speed-rpm",
        type=float,
        default=float(_param(params, "scan_home_speed_rpm", 160.0)),
    )
    parser.add_argument(
        "--scan-prepare-speed-rpm",
        type=float,
        default=float(_param(params, "scan_prepare_speed_rpm", 30.0)),
    )
    parser.add_argument(
        "--scan-center-extra-time-s",
        type=float,
        default=float(_param(params, "scan_center_extra_time_s", 0.05)),
    )
    parser.add_argument(
        "--scan-timeout-s", type=float, default=float(_param(params, "scan_timeout_s", 5.0))
    )
    parser.add_argument(
        "--scan-sample-period-s",
        type=float,
        default=float(_param(params, "scan_sample_period_s", 0.02)),
    )
    parser.add_argument(
        "--scan-stop-action", default=str(_param(params, "scan_stop_action", "prepare"))
    )
    parser.add_argument(
        "--scan-stop-args",
        nargs=4,
        type=float,
        default=list(_param(params, "scan_stop_args", [0.0, 0.0, 0.0, 0.0])),
    )
    parser.add_argument(
        "--scan-stop-timeout-s",
        type=float,
        default=float(_param(params, "scan_stop_timeout_ms", 3000)) / 1000.0,
    )
    parser.add_argument("--service-wait-timeout-s", type=float, default=3.0)
    parser.add_argument(
        "--forward-speed-mps", type=float, default=float(_param(params, "forward_speed_mps", 0.2))
    )
    parser.add_argument(
        "--forward-duration-s", type=float, default=float(_param(params, "forward_duration_s", 1.8))
    )
    parser.add_argument(
        "--direction-sign-y", type=float, default=float(_param(params, "direction_sign_y", -1.0))
    )
    parser.add_argument(
        "--grasp-timeout-s",
        type=float,
        default=float(_param(params, "grasp_timeout_ms", 15000)) / 1000.0,
    )
    parser.add_argument(
        "--lift-height-mm",
        nargs=4,
        type=float,
        default=list(_param(params, "lift_height_mm", [70.0, 70.0, 70.0, 70.0])),
    )
    parser.add_argument(
        "--lower-height-mm",
        nargs=4,
        type=float,
        default=list(_param(params, "lower_height_mm", [28.0, 28.0, 28.0, 28.0])),
    )
    parser.add_argument(
        "--retreat-speed-mps", type=float, default=float(_param(params, "retreat_speed_mps", 0.2))
    )
    parser.add_argument(
        "--retreat-duration-s", type=float, default=float(_param(params, "retreat_duration_s", 1.0))
    )
    parser.add_argument(
        "--publish-rate-hz", type=float, default=float(_param(params, "publish_rate_hz", 100.0))
    )
...
def _print_config(args: argparse.Namespace) -> None:
    ...
    print(
        "  pre-scan position = prepare([%.4f, %.4f, 0.0, 0.0])"
        % (
    ...
    print(
        "  scan target = prepare([%.4f, %.4f, 0.0, 0.0])"
        % (
    ...
    print(
        "  present trigger: distance <= %.3f mm for %.3f s"
        % (
    ...
    print(
        "  jump trigger: %s, abs(delta) >= %.3f mm"
        % (
    ...
    print(
        "  forward: speed %.3f * sign %.1f for %.3f s"
        % (
...
def _show_sensor(node: SensorScanDebugCli, label: str = "传感器快照") -> bool:
    ...
    print(
        "  all distances = %s"
        % [None if not math.isfinite(value) else round(value, 3) for value in distances]
    )
    print(
        "  sensor[%d] = %s mm, age = %.3f s, valid = %s"
        % (
...
def _monitor_scan(node: SensorScanDebugCli) -> dict[str, Any] | None:
    ...
    while time.monotonic() < deadline_s and rclpy.ok():
        ...
        delta_mm = math.nan
        if args.scan_enable_jump_trigger and node.is_valid_scan_distance(
...
def _run_debug_flow(node: SensorScanDebugCli) -> None:
    ...
    if not _ask("步骤 2/8: 是否先把夹爪运动到 %.4fm 位置？" % (args.debug_pre_scan_position_m,)):
        print("取消本轮。")
        return
    ...
    if not _ask("步骤 3/8: 是否启动慢速扫描到 %.4fm 位置？" % (args.debug_scan_target_position_m,)):
        print("取消本轮。")
        return
    ...
    if _ask("步骤 4/8: 是否执行 FORWARD 前进？"):
        ...
        print(
            "FORWARD: speed=%.3fm/s duration=%.3fs"
            % (
                speed,
                args.forward_duration_s,
            )
    ...
    if _ask("步骤 7/8: 是否执行 RETREAT 后退？"):
        ...
        print(
            "RETREAT: speed=%.3fm/s duration=%.3fs"
            % (
                speed,
