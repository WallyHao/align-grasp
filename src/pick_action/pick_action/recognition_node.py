...
class RecognitionNode(Node):
    ...
    def __init__(self) -> None:
        ...
        values = {name: self.get_parameter(name).value for name in defaults.__dict__}
        self._config = ProcessorConfig(**values)
        self._grids = self._new_grids()
        self._json_pub = self.create_publisher(String, "spear_recognition/result", 10)
        self._marker_pub = self.create_publisher(MarkerArray, "spear_recognition/markers", 10)
    ...
    def _new_grids(self):
        return [
            (
                "standard",
                TemporalGrid(resolution_m=float(self.get_parameter("grid_resolution_m").value)),
    ...
    def _callback(self, scan: LaserScan) -> None:
        ...
        if self._grids[0][1].frame_count < int(self.get_parameter("window_frames").value):
            return
        ...
        targets = []
        standard_components = []
        maximum_span = float(self.get_parameter("maximum_component_span_m").value)
        profile_solutions = []
        profile_penalties = {
            "standard": 0.0,
            "near_split": 0.0005,
        ...
        for profile, grid, occupancy, connection_radius in self._grids:
            ...
            profile_targets = select_target_components(
                ...
                maximum_spacing_deviation_ratio=float(
                    self.get_parameter("maximum_spacing_deviation_ratio").value
                ),
                alignment_axis="auto",
            )
            if (
            ...
            if len(profile_targets) == expected_count:
                score = arrangement_alignment_error(profile_targets) + profile_penalties[profile]
                profile_solutions.append((score, profile, profile_components, profile_targets))
        if profile_solutions:
            _, selected_profile, components, targets = min(profile_solutions)
        raw_targets = targets
        if len(raw_targets) == expected_count and len(raw_targets) == len(
    ...
    def _publish(
        ...
        spacing = arrangement_spacing_metrics(targets) if len(targets) >= 2 else None
        message = String()
        message.data = json.dumps(
            {
                ...
                "status": ("recognized" if len(targets) == expected_count else "not_separable"),
                ...
                "mean_adjacent_spacing_m": (round(spacing[0], 6) if spacing is not None else None),
                ...
                "targets": [
                    ...
                    for index, (raw_target, target) in enumerate(
                        zip(raw_targets, targets, strict=True)
                    )
                ],
            },
            ensure_ascii=False,
        ...
        for index, target in enumerate(targets):
            ...
            label.color.b = 1.0
            label.color.a = 1.0
            label.text = f"ID {index}\nx={target.x_m:.3f} y={target.y_m:.3f}"
